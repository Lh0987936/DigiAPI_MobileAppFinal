...
class DigiListViewModel: ObservableObject {
    @Published var content: [DigiFile] = []
    @Published var selected: Digimon? = nil

    func DigiScan() {
        guard let url = URL(string: "https://digi-api.com/api/v1/digimon?page=100") else {
            print("Invalid URL")
            return
        }
